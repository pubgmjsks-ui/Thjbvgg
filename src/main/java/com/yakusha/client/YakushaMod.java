...
public class YakushaMod implements ClientModInitializer {
    ...
    public static class YakushaGui extends Screen {
        ...
        public boolean mouseClicked(double mx, double my, int b){
            if(b==0){int y=py+20;for(Feature f:features){if(mx>=px+5&&mx<=px+pw-5&&my>=y&&my<=y+12){f.toggle();return true;}y+=14;}}
            return super.mouseClicked(mx,my,b);
        }
        public boolean keyPressed(int k,int s,int mod){if(k==GLFW.GLFW_KEY_RSHIFT){mc.setScreen(null);return true;}return super.keyPressed(k,s,mod);}
    }
    ...
    public static class AttackAura extends Feature {
        ...
        public void tick(){
            ...
            for(Entity e:mc.world.getEntities())if(e instanceof LivingEntity&&e!=mc.player&&!e.isDead()&&mc.player.distanceTo(e)<md&&mc.player.canSee(e)){t=e;md=mc.player.distanceTo(e);}
            if(t==null)return;
            if(mc.player.getAttackCooldownProgress(0.5f)>=1.0f){mc.interactionManager.attackEntity(mc.player,t);mc.player.swingHand(Hand.MAIN_HAND);}
    ...
    public static class TriggerBot extends Feature {
        ...
        public void tick(){
            if(mc.crosshairTarget instanceof EntityHitResult){Entity e=((EntityHitResult)mc.crosshairTarget).getEntity();if(e instanceof LivingEntity&&e!=mc.player&&!e.isDead()&&mc.player.getAttackCooldownProgress(0.5f)>=1.0f){mc.interactionManager.attackEntity(mc.player,e);mc.player.swingHand(Hand.MAIN_HAND);}}
        }
    }
    ...
    public static class JumpCircle extends Feature {
        ...
        public void tick(){
            ...
            if(ct>0){for(int i=0;i<8;i++){double a=i*Math.PI/4;mc.world.addParticle(net.minecraft.particle.ParticleTypes.FLAME,mc.player.getX()+Math.cos(a)*0.8,mc.player.getY()+0.2,mc.player.getZ()+Math.sin(a)*0.8,0,0.02,0);}ct--;}
        }
    }
    ...
    public static class AutoFarm extends Feature {
        ...
        public void tick(){
            ...
            c++;if(c%4!=0)return;
            for(int x=-4;x<=4;x++)for(int z=-4;z<=4;z++)for(int y=-1;y<=1;y++){var p=mc.player.getBlockPos().add(x,y,z);var s=mc.world.getBlockState(p);if(s.getBlock()==net.minecraft.block.Blocks.WHEAT&&s.get(net.minecraft.block.CropBlock.AGE)>=7)mc.interactionManager.attackBlock(p,net.minecraft.util.math.Direction.UP);}
        }
    }

    public static class Fly extends Feature {
        public Fly(){super("Fly");}
        public void tick(){if(mc.player!=null&&mc.options.keyJump.isPressed()){mc.player.setVelocity(mc.player.getVelocity().x,0.15,mc.player.getVelocity().z);mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(),mc.player.getY()+0.15,mc.player.getZ(),false));}}
    }

    public static class Speed extends Feature {
        public Speed(){super("Speed");}
        public void tick(){if(mc.player!=null&&mc.player.isOnGround()&&mc.options.keyForward.isPressed()){float y=mc.player.yaw;mc.player.setVelocity(mc.player.getVelocity().add(Math.sin(Math.toRadians(-y))*0.05,0,Math.cos(Math.toRadians(-y))*0.05));}}
    }
    ...
    public static class AutoTotem extends Feature {
        ...
        public void tick(){
            if(mc.player!=null&&mc.player.getOffHandStack().getItem()!=Items.TOTEM_OF_UNDYING)for(int i=0;i<36;i++)if(mc.player.inventory.getStack(i).getItem()==Items.TOTEM_OF_UNDYING){mc.interactionManager.pickFromInventory(i);mc.interactionManager.pickFromInventory(40);break;}
        }
    }
                                  }
